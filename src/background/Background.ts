...
import { InitialState } from "@/apps/iframe/store/store";
...
async function getInitialOptionsState(): Promise<OptionsState> {
  const ans = new DeferredPromise<OptionsState>();
  browser.storage.local.get("options").then(async res => {
    const storedOptions: OptionsState = res.options;
    if (storedOptions && !storedOptions.guid) {
      ...
      ans.resolve({ ...InitialState.OptionsState, ...newOptions });
    } else if (storedOptions) {
      ans.resolve({ ...InitialState.OptionsState, ...storedOptions });
    } else {
      ans.resolve(InitialState.OptionsState);
    }
  });
  return ans;
}
...
getInitialOptionsState().then(
  optionsState => (appState.OptionsState = optionsState),
);
